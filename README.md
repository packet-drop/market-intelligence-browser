...
The service implements manual Seeking Alpha session bootstrap, encrypted persistence, and a
hydration-aware ticker lookup for Seeking Alpha Quant Ratings. It does not automate login or expose
...
| `POST` | `/api/sources/seeking-alpha/quant-ratings/lookup` | Service bearer token                    | Read a hydrated Quant Rating and observed price for a validated ticker |
...
| `SEEKING_ALPHA_NAVIGATION_TIMEOUT_MS`      | `30000`                           | Navigation and client-side hydration timeout                                  |
...
Look up a Quant Rating with a ticker—not a URL:

```bash
curl -X POST http://localhost:3000/api/sources/seeking-alpha/quant-ratings/lookup \
  -H "Authorization: Bearer $SERVICE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"ticker":"AAPL"}'
```

Tickers are trimmed, uppercased, and restricted to a 1–15 character symbol grammar supporting
letters, digits, dots, and hyphens. The server constructs the canonical Seeking Alpha path. A
successful result contains `ticker`, normalized `rating`, numeric `score`, `observedPrice`,
`canonicalPath`, and `observedAt`. Operational failures use bounded codes such as
`SESSION_EXPIRED`, `CHALLENGE_REQUIRED`, `HYDRATION_TIMEOUT`, `UNSUPPORTED_STATE`, and
`SELECTOR_DRIFT`; no upstream URL or raw HTML is returned.
...
Complete login manually in the isolated Google Chrome profile and press Enter in the terminal. The
temporary profile is removed after import. Verify the session through
